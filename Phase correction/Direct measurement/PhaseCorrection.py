...
'''Avant de runer le code principal, executer la cellule qui contient la bibliothèque ALP car je n'arrive pas à utiliser correctement le header'''


# In[23]:


#runner cette cellule si le code a été arrété 
...
background=capture()
basedmd=np.zeros((768,1024),dtype=int) #initialisation du tableau dans lequel on entrera l'image ref de phase
...
for i in range(dm//4):
    for j in range(dm):
        basedmd[(nm//2)*dm+j,128+(nm//2)*dm+4*i]=255
        basedmd[(nm//2)*dm+j,128+(nm//2)*dm+4*i+1]=255
