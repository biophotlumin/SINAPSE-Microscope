...
def holo(wavelentgh,x_ech,y_ech,z_ech,corr):
    ...
    def f(i, j):
        x=(i+1/2-i0)*d
        y=(j+1/2-j0)*d
        return ((1+cos((2*pi/wavelentgh)*(sqrt((C[0][0]-x)**2+(C[1][0]-y)**2+C[2][0]**2)-sr*x-cr*y)-corr[i,j]))>0.5)
...
plt.imshow(basedmd)
plt.show()
#ax1.set_xlim([743,773])
#ax1.set_ylim([607,637])



COEFF=[0.,-0.6,0.,-0.9,-0.5,-0.8,1.1,-1.5,-1.4,-1.4,1.4,0.,0.,-1.2,1.4]
complex_mask = complex_mask_from_zernike_coeff(shape = [npx,npy],
                                   radius = int(sigma_beam/d),
                                   center = [npx//2,npy//2],vec= COEFF)
dmdcorrige=holo(lam,0,0,0,angle(complex_mask))
save("C:\\Users\\lac\\Desktop\\Florian\\git\\SINAPSE-Microscope\\Phase correction\\PhaseMask",angle(complex_mask))
afficher(dmdcorrige,DMD)
plt.imshow(dmdcorrige)
plt.show()
