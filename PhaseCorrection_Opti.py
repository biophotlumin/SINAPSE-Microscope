...
lam =0.515e-6 #Longueure d'onde en METRE
...
def holo(wavelentgh,x_ech,y_ech,z_ech,corr):
    ...
    s=1/sqrt(2)
    cp=sqrt(cos(theta)**2-sin(theta)*r*wavelentgh*cos(rho)/(4*d)-(r*wavelentgh/(8*d))**2) #cp=cos(phi)
    pc=arccos(cp)*180/pi #pc=phi juste pour avoir une idée de sa valeur
    sp=sqrt(sin(theta)**2+sin(theta)*r*wavelentgh*cos(rho)/(4*d)+(r*wavelentgh/(8*d))**2)
    ps=arcsin(sp)*180/pi
    phi=arctan(sp/cp)*180/pi
    nx=(-r*wavelentgh/(8*d)-sin(theta)*cos(rho))/sp
    ...
    def f(i, j):
        x=(i+1/2-i0)*d
        y=(j+1/2-j0)*d
        return ((1+cos((2*pi/wavelentgh)*(sqrt((C[0][0]-x)**2+(C[1][0]-y)**2+C[2][0]**2)-sr*x-cr*y)-corr[i,j]))>1.5)
...
def get_cost(img,mask_center, mask_radius = 3):
    res = img.shape
    ...
    mask = exp(-((X-mask_center[0])**2+(Y-mask_center[1])**2)/(2*mask_radius**2))
    signal = sum(img*mask)/sum(mask)
...
#COEFF=[0.,1.,-0.4,1.,-0.3,-1.3,-0.2,0.7,-1.3,-1.3,0.,1.4,-0.8,0.7,0.,0.]
COEFF=[0.,-0.6,0.,-1.,-0.4,-0.8,0.,0.,-0.2,-1.3,0.,0.7,0.,-1.2,1.4]
complex_mask = complex_mask_from_zernike_coeff(shape = [npx,npy],
                                   radius = int(sigma_beam/d),
                                   center = [npx//2,npy//2],vec= COEFF)
...
coeff=0
ntest=30
for cz in range (3,12):
    cmax=0
    for test in range(ntest):
        COEFF[cz]=-1.5+3/ntest*test
        complex_mask = complex_mask_from_zernike_coeff(shape = [npx,npy],
                                           radius = int(sigma_beam/d),
                                           center = [npx//2,npy//2],vec= COEFF)
        imgdmd=holo(lam,0,0,0,angle(complex_mask))
        afficher(imgdmd,DMD)
        lum=capture()
        c=get_cost(lum,[680,644]) #

        if c>cmax :
            cmax=c
            coeff=-1.5+3/ntest*test
    COEFF[cz]=coeff
...
ax1.imshow(basespot)
ax1.set_xlim(660,708)
ax1.set_ylim(637,685)
ax1.set_title('spot avant correction')
ax2=plt.subplot(2,2,2)
complex_maskfinal = complex_mask_from_zernike_coeff(shape = [npx,npy],
                                   radius = int(sigma_beam/d),
                                   center = [npx//2,npy//2],vec= COEFF)
dmdfinal=holo(lam,0,0,0,angle(complex_maskfinal))
afficher(dmdfinal,DMD)
lum=capture()

ax2.imshow(lum)
ax2.set_xlim(660,708)
ax2.set_ylim(637,685)
ax2.set_title('spot après correction')
ax2.set_xlabel(str(COEFF))

plt.subplot(2,2,3)
plt.imshow(angle(complex_maskfinal))
...
afficher(basedmd,DMD)
basespot=capture()
c=get_cost(basespot,[758,622])
ax=plt.subplot(2,1,1)
plt.imshow(basespot)
plt.text(200,200,'coeffs = '+str(COEFF)+' et c = '+str(c))


COEFF=[0.,1.3,-0.3,0.7,1.5,-2.,0.2,0.,0.,0.]
complex_mask = complex_mask_from_zernike_coeff(shape = [npx,npy],
                                            radius = int(sigma_beam/d),
                                            center = [npx//2,npy//2],vec=COEFF )
basedmd=holo(lam,0,0,0,angle(complex_mask))
